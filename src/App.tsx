...
try {
  ...
  if (typeof w.__firebase_config !== "undefined" && w.__firebase_config) {
    firebaseConfig =
      typeof w.__firebase_config === "string"
...
if (firebaseConfig && Object.keys(firebaseConfig).length > 0) {
  try {
    ...
    // TẠI SAO (Why): Thừa hưởng phương pháp đồng bộ offline-first mượt mà của David East.
    // Tự động bật persistent cache lưu trữ ngoại tuyến cục bộ (IndexedDB) hỗ trợ đồng bộ hóa đa tab/iframe an toàn.
    // Điều này triệt tiêu hoàn toàn lỗi "failed-precondition" (khi mở ứng dụng ở nhiều tab cùng lúc trong iframe phát triển),
    // đồng thời mang lại trải nghiệm tải tức thì (0ms latency) từ cache cục bộ cho thiết bị di động.
    try {
      ...
      console.log("Firestore initialized successfully with multi-tab persistent cache.");
    } catch (cacheErr) {
      console.warn("Could not initialize with multi-tab persistence, falling back to standard Firestore:", cacheErr);
...
const DATA_URL =
  "https://raw.githubusercontent.com/vietvudanh/vietlott-data/main/data/bingo18.jsonl";
...
export default function App() {
  ...
  const [arParams, setArParams] = useState<ARParams>({
    lag: 5,
    emaAlpha: 0.3,
    learningRate: 0.01,
    epochs: 150,
  });
  const [mlpParams, setMlpParams] = useState<MLPParams>({
    inputLags: 5,
    hiddenNeurons: 8,
    learningRate: 0.05,
    epochs: 250,
  ...
  useEffect(() => {
    if (driveToken && driveSyncStatus === "idle") {
      ...
      bottomProfiler.profileOperation("google_drive_sync", () =>
        syncWithGoogleDrive(driveToken, manualData)
      )
        .then((merged) => {
  ...
  const saveManualData = useCallback(
    async (newList: Draw[]) => {
      ...
      if (driveToken) {
        setDriveSyncStatus("syncing");
        bottomProfiler.profileOperation("google_drive_sync", () =>
          syncWithGoogleDrive(driveToken, newList)
        )
          .then(() => {
            setDriveSyncStatus("synced");
          })
      ...
      if (db && user) {
        ...
        bottomProfiler.profileOperation("firestore_sync", async () => {
          const docRef = doc(
            db,
            "artifacts",
            appId,
  ...
  const fetchData = useCallback(
    async (isInitialLoad = false, isSilent = false) => {
      ...
      try {
        ...
        if (isInitialLoad && db && auth?.currentUser) {
          try {
            ...
            if (cacheSnap.exists() && Array.isArray(cacheSnap.data().list)) {
              const restoredList: Draw[] = cacheSnap
                .data()
                .list.map((rawStr: string) => {
              ...
              setFetchedData((prev) =>
                prev.length === 0 ? restoredList : prev,
              );
            }
          } catch (e) {
            console.warn(
              "Firestore Cache load failed, proceeding with direct fetch",
            );
          }
        }

        const response = await fetchWithRetry(`${DATA_URL}?t=${new Date().getTime()}`);
        ...
        const parsedData: Draw[] = targetLines
          .map((line, index) => {
            try {
        ...
        if (db && auth?.currentUser && parsedData.length > 0) {
          ...
          const compressedCache = parsedData.map(
            (d) => `${d.id}|${d.numbers.join(",")}|${d.date}`,
          );
        ...
        if (parsedData.length > 0) {
          setManualData((currentManual) => {
            const manualMap = new Map<string, Draw>(
              currentManual.map((d) => [String(d.id), d]),
            );
            let updated = false;
        ...
        if (!isInitialLoad && !isSilent) {
          showToast(
            "Đã đồng bộ bộ dữ liệu Deep-Quant V9.0 mới nhất!",
            "success",
          );
        }
      } catch (err: any) {
        ...
        if (isInitialLoad) {
          ...
          setError(
            "Không kết nối được máy chủ Vietlott API. Đang tải mô hình Offline.",
          );
        } else if (!isSilent) {
          showToast(
            "Lỗi cập nhật. Hãy kiểm tra kết nối mạng của bạn.",
            "error",
          );
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const unsubManual = onSnapshot(
      manualDocRef,
      { includeMetadataChanges: true }, // TẠI SAO (Why): Theo dõi sự thay đổi metadata (từ cache hay từ server) như David East khuyến nghị để tối ưu hóa đồng bộ
      (docSnap) => {
        if (docSnap.exists() && Array.isArray(docSnap.data().list)) {
          setManualData(
            docSnap
              .data()
              .list.map((item: any) => ({ ...item, isManual: true })),
          );
        }
        
        // TẠI SAO (Why): Phân tích trạng thái đồng bộ hóa thời gian thực (real-time latency tracking)
        // Nếu có thay đổi chưa ghi xong lên server -> "syncing"
        // Nếu tải từ cache ngoại tuyến cục bộ -> "synced" (hiển thị 0ms từ IndexedDB)
      ...
      (err) => {
        console.warn(
          "Firestore snapshot error, falling back to local cache",
          err,
        );
        setCloudSyncStatus("error");
      },
  ...
  const handleAddManualData = useCallback(() => {
    ...
    if (combinedData.some((d) => String(d.id) === String(newDraw.id))) {
      return showToast(
        `Mã kỳ quay #${newDraw.id} đã tồn tại trong hệ thống!`,
        "error",
      );
    }
  ...
  const handleBulkAddManualData = useCallback((newDraws: Draw[]) => {
    if (newDraws.length === 0) return;

    const existingIds = new Set(combinedData.map((d) => String(d.id)));
    const filteredNewDraws = newDraws.filter(
      (d) => !existingIds.has(String(d.id))
    );

    if (filteredNewDraws.length === 0) {
      showToast("Tất cả các kỳ quay trong dữ liệu đã tồn tại trong hệ thống!", "info");
      return;
    }
  ...
  const handleRemoveManualData = useCallback(
    (idToRemove: string) => {
      const updated = manualData.filter(
        (d) => String(d.id) !== String(idToRemove),
      );
      saveManualData(updated);
  ...
  const filteredData = useMemo(() => {
    ...
    return combinedData.filter((draw) => {
      ...
      if (
        cleanQuery &&
        !draw.id.toString().toLowerCase().includes(cleanQuery)
      ) {
        return false;
      }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans select-none selection:bg-indigo-500/30">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <h1 className="text-slate-100 text-lg font-black tracking-tight">
                  BINGO18 Deep-Quant
                </h1>
              ...
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                Hệ thống phân tích & dự đoán Vietlott
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3.5 justify-start sm:justify-end">
            ...
            <div className="flex items-center gap-2 bg-slate-950/60 border border-slate-900 rounded-xl px-3.5 py-2">
              ...
              <div className="text-right">
                <div className="text-slate-200 text-xs font-black font-mono leading-none">
                  {timeString}
                </div>
                <div className="text-[9px] text-slate-500 font-medium leading-none mt-1 whitespace-nowrap">
                  {dateString}
                </div>
              </div>
            </div>
            ...
            <button
              onClick={() => {
                ...
                showToast(
                  nextState
                    ? "Đã kích hoạt tự động cập nhật thời gian thực (12 giây)!"
                    : "Đã tắt tự động cập nhật thời gian thực.",
                  "info",
                );
              }}
            ...
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${autoRefresh ? "bg-emerald-400 animate-ping" : "bg-slate-600"}`}
              />
              ...
              onClick={() => {
                if (!driveToken) {
                  window.parent.postMessage(
                    { type: "REQUEST_OAUTH_TOKEN" },
                    "*",
                  );
                  showToast("Đang kết nối Google Drive...", "info");
                } else {
                  setDriveSyncStatus("syncing");
                  bottomProfiler.profileOperation("manual_google_drive_sync", () =>
                    syncWithGoogleDrive(driveToken, manualData)
                  )
                    .then((merged) => {
                      if (merged.length > manualData.length) {
            ...
            >
              <Database
                className={`w-3.5 h-3.5 ${driveSyncStatus === "syncing" ? "animate-pulse" : ""}`}
              />
        ...
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center py-32 text-center">
            ...
            <p className="text-slate-400 font-bold text-sm">
              Đang nạp dữ liệu kỳ quay Bingo18...
            </p>
            <p className="text-slate-600 text-xs mt-1">
              Khởi tạo và tính toán mô phỏng lượng tử định lượng.
            </p>
          </div>
        ) : (
      ...
      <footer className="bg-slate-950 border-t border-slate-900 shrink-0 px-4 py-5 text-center text-[10px] text-slate-600 font-bold uppercase tracking-wider">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-3">
          <span>
            Bingo18 Predictor & Analyzer © 2026 • Deep-Quant V9.0 engine
          </span>
          <span className="flex items-center gap-1.5 justify-center text-slate-500 font-semibold text-[9px]">
            <ShieldAlert className="w-3.5 h-3.5 text-amber-500" />
            Dự án nghiên cứu toán học xác suất lý thuyết • Không chịu trách
            nhiệm kết quả thực tế
          </span>
        </div>
      </footer>
      ...
      {toast.show && (
        <div
          className={`fixed bottom-5 right-5 z-50 flex items-center gap-2.5 px-4.5 py-3 rounded-xl border shadow-2xl transition-all duration-300 transform scale-100 font-bold text-xs ${
            toast.type === "success"
