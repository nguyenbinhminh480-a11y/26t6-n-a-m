    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center bg-slate-900/40 rounded-2xl border border-slate-800">
        ...
        <p className="text-slate-400 font-medium">
          Chưa có đủ số liệu thống kê.
        </p>
        <p className="text-slate-500 text-sm mt-1">
          Vui lòng nạp dữ liệu kỳ quay để biểu diễn các chỉ số đồ họa.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900/40 backdrop-blur-md p-5 rounded-2xl border border-slate-800 flex flex-col justify-between">
          ...
          <div className="w-full bg-slate-950/80 h-3 rounded-full flex overflow-hidden mt-6 border border-slate-800">
            <div
              ...
              style={{ width: `${analytics.taiPercentage}%` }}
            />
            <div
              className="bg-amber-500 h-full"
              style={{ width: `${analytics.hoaPercentage}%` }}
            />
            <div
              className="bg-sky-500 h-full"
              style={{ width: `${analytics.xiuPercentage}%` }}
            />
          </div>
        </div>
        ...
        <div className="bg-slate-900/40 backdrop-blur-md p-5 rounded-2xl border border-slate-800 flex flex-col justify-between">
          <div>
            ...
            <div className="flex justify-around items-center mt-3">
              <div className="text-center">
                <div className="text-indigo-400 text-2xl font-black font-mono">
                  {analytics.evenPercentage}%
                </div>
              ...
              <div className="text-center">
                <div className="text-amber-500 text-2xl font-black font-mono">
                  {analytics.oddPercentage}%
                </div>
                <div className="text-[10px] text-slate-500 font-bold mt-1">
                  TỔNG LẺ
                </div>
          ...
          <div className="w-full bg-slate-950/80 h-3 rounded-full flex overflow-hidden mt-6 border border-slate-800">
            <div
              ...
              style={{ width: `${analytics.evenPercentage}%` }}
            />
            <div
              className="bg-amber-500 h-full"
              style={{ width: `${analytics.oddPercentage}%` }}
            />
          </div>
        </div>
        ...
        <div className="bg-slate-900/40 backdrop-blur-md p-5 rounded-2xl border border-slate-800">
          <h3 className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-4">
            Mặt số Xuất hiện nhiều/ít nhất
          </h3>
            ...
            <div className="bg-rose-500/5 p-3 rounded-xl border border-rose-500/10">
              ...
              <div className="flex items-center gap-1.5 mt-2">
                {analytics.hotNumbers.map((num, i) => (
                  <div
                    key={i}
                    className="w-8 h-8 bg-rose-600 text-slate-100 rounded-lg flex items-center justify-center font-extrabold text-sm shadow"
                  >
                    {num}
                  </div>
                ))}
            ...
            <div className="bg-sky-500/5 p-3 rounded-xl border border-sky-500/10">
              ...
              <div className="flex items-center gap-1.5 mt-2">
                {analytics.coldNumbers.map((num, i) => (
                  <div
                    key={i}
                    className="w-8 h-8 bg-sky-600 text-slate-100 rounded-lg flex items-center justify-center font-extrabold text-sm shadow"
                  >
                    {num}
                  </div>
                ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900/40 backdrop-blur-md p-5 rounded-2xl border border-slate-800 shadow-lg">
          <div className="mb-4">
            <h3 className="text-slate-200 font-bold text-sm">
              Biểu đồ Tần suất Tổng Điểm (Sum Distribution)
            </h3>
            <p className="text-slate-500 text-xs mt-0.5">
              Tỷ lệ phần trăm xuất hiện của các tổng từ 3 đến 18 điểm
            </p>
          </div>
              ...
              >
                ...
                <Bar dataKey="percentage" radius={[4, 4, 0, 0]}>
                  {sumChartData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={getSumTypeColor(entry.sumRaw)}
                    />
                  ))}
                </Bar>
              </BarChart>
        ...
        <div className="bg-slate-900/40 backdrop-blur-md p-5 rounded-2xl border border-slate-800 shadow-lg">
          <div className="mb-4">
            <h3 className="text-slate-200 font-bold text-sm">
              Biểu đồ Tần suất Mặt Xúc Xắc
            </h3>
            <p className="text-slate-500 text-xs mt-0.5">
              So sánh mức độ xuất hiện đồng đều của các mặt xúc xắc từ 1 đến 6
            </p>
          </div>
        ...
        <div className="lg:col-span-8 bg-slate-900/40 backdrop-blur-md p-5 rounded-2xl border border-slate-800 shadow-lg">
          <div className="mb-4">
            <h3 className="text-slate-200 font-bold text-sm flex items-center gap-2">
              <GitMerge className="w-4 h-4 text-indigo-400" /> Ma trận đồng xuất
              hiện xúc xắc (Dice Co-occurrence Affinity)
            </h3>
            <p className="text-slate-500 text-xs mt-0.5">
              Xác định cặp mặt số thường được lắc ra cùng nhau trong cùng một kỳ
              quay
            </p>
          </div>
          ...
          <div className="grid grid-cols-7 gap-1.5 max-w-lg mx-auto">
            {/* Header label corner */}
            <div className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center text-[10px] text-slate-500 font-bold font-mono">
              DICE
            </div>
            {/* Column labels */}
            {[1, 2, 3, 4, 5, 6].map((col) => (
              <div
                key={col}
                className="w-8 h-8 sm:w-10 sm:h-10 bg-slate-850/60 rounded flex items-center justify-center font-bold text-xs text-indigo-300 border border-slate-800"
              >
                {col}
              </div>
            ))}
                ...
                {[1, 2, 3, 4, 5, 6].map((col, colIdx) => {
                  ...
                  if (!isSelf) {
                    if (score > 80)
                      ...
                      cellBg = "bg-indigo-500/70 text-indigo-100";
                    else if (score > 40)
                      cellBg = "bg-indigo-500/40 text-indigo-200";
                    else if (score > 20)
                      cellBg = "bg-indigo-500/15 text-indigo-300";
                    else cellBg = "bg-slate-900/30 text-slate-400";
                  } else {
                    cellBg =
                      "bg-slate-800/20 text-slate-600 border border-dashed border-slate-800";
                  }
                  ...
                  return (
                    <div
                      ...
                      title={
                        isSelf
                          ? `Mặt ${row}`
                          : `Mặt ${row} & ${col} có độ thân thuộc ${score}%`
                      }
                    >
                      {isSelf ? "•" : score}
                    </div>
          ...
          <div className="flex items-center gap-4 mt-5 text-[10px] text-slate-500 font-semibold justify-center">
            <div className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-indigo-500/15 inline-block" />{" "}
              Thấp
            </div>
            <div className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-indigo-500/40 inline-block" />{" "}
              Trung bình
            </div>
            <div className="flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded bg-indigo-500/70 inline-block" />{" "}
              Cao
            </div>
            <div className="flex items-center gap-1">
        ...
        <div className="lg:col-span-4 bg-slate-900/40 backdrop-blur-md p-5 rounded-2xl border border-slate-800 shadow-lg flex flex-col justify-between">
          <div>
            <h3 className="text-slate-200 font-bold text-sm mb-4 flex items-center gap-2">
              <Award className="w-4 h-4 text-amber-400" /> Thống kê chuỗi bệt
              cực đại
            </h3>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-slate-950/40 rounded-xl border border-slate-850">
                ...
                <div className="font-mono text-slate-200 font-black text-sm">
                  {analytics.maxTaiStreak} kỳ liên tiếp
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-slate-950/40 rounded-xl border border-slate-850">
                ...
                <div className="font-mono text-slate-200 font-black text-sm">
                  {analytics.maxXiuStreak} kỳ liên tiếp
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-slate-950/40 rounded-xl border border-slate-850">
                ...
                <div className="font-mono text-slate-200 font-black text-sm">
                  {analytics.maxHoaStreak} kỳ liên tiếp
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 p-4 bg-amber-500/5 border border-amber-500/10 rounded-xl text-[11px] text-slate-400 leading-relaxed font-medium">
            ...
            Khi chuỗi bệt đạt trên 5 kỳ liên tiếp, hãy theo dõi chỉ số độ biến
            động (SD). Nếu SD giảm mạnh xuống dưới 1.1, cầu có thể tiếp tục bệt
            dài (gọi là bệt rồng). Ngược lại, nếu SD tăng vọt thì khả năng cầu
            nhảy đảo ngược là cực kỳ cao.
          </div>
        </div>
      </div>
    </div>
