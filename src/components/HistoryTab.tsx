...
import { MinerUDataParser } from "../utils/minerUParser";
...
interface HistoryTabProps {
  ...
  onBulkAddManualData: (draws: Draw[]) => void;
...
export const HistoryTab: React.FC<HistoryTabProps> = ({
  ...
  const handleBulkParse = () => {
    ...
    const res = MinerUDataParser.parseUnstructuredText(bulkText);
    setParseResult(res);
  };

  const handleBulkImportSubmit = () => {
    if (!parseResult || parseResult.draws.length === 0) return;
    onBulkAddManualData(parseResult.draws);
    setBulkText("");
    setParseResult(null);
  };
  ...
  const getDiceDotClass = (val: number) => {
    switch (val) {
      ...
      case 4:
        return "col-start-1 row-start-1, col-start-1 row-start-3, col-start-3 row-start-1, col-start-3 row-start-3";
      case 5:
        return "col-start-1 row-start-1, col-start-1 row-start-3, col-start-2 row-start-2, col-start-3 row-start-1, col-start-3 row-start-3";
      case 6:
        return "col-start-1 row-start-1, col-start-1 row-start-2, col-start-1 row-start-3, col-start-3 row-start-1, col-start-3 row-start-2, col-start-3 row-start-3";
      default:
        return "";
    }
  };
        ...
        <div className="bg-slate-900/60 backdrop-blur-md p-6 rounded-2xl border border-slate-800 shadow-2xl relative overflow-hidden animate-in fade-in duration-300">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 pb-4 border-b border-slate-800/60">
            <h3 className="text-slate-100 font-extrabold text-sm flex items-center gap-2">
              <Plus className="w-4 h-4 text-indigo-400 animate-pulse" /> Nhập kết quả tự tạo
            </h3>
          ...
          {inputMode === "single" ? (
            ...
            <div className="space-y-4">
              <div className="text-[11px] text-slate-400 bg-slate-950/40 p-3 rounded-xl border border-slate-800">
                <p className="font-extrabold text-indigo-400 mb-1 uppercase tracking-wider flex items-center gap-1">
                  <Sparkles className="w-3.5 h-3.5" /> Trình trích xuất MinerU Unstructured Document Parser
                </p>
                Dán kết quả thô được sao chép trực tiếp từ bảng PDF, trang web, tệp CSV hoặc văn bản không định dạng. 
                Hệ thống tự động bỏ nhiễu, chuẩn hóa cột xúc xắc, nhận diện mã kỳ quay và căn chỉnh cấu trúc lệch.
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                <div className="lg:col-span-7">
                  <textarea
                    rows={6}
                    placeholder={`Ví dụ sao chép thô:\nKỳ 00985: 3, 5, 2\n00986 | 2026-06-25 | 1-4-6\nCột nhiễu: 987 5 3 2\n[00988] 6 6 6`}
                    value={bulkText}
                    onChange={(e) => {
                      setBulkText(e.target.value);
                      if (parseResult) setParseResult(null);
                    }}
                    className="w-full bg-slate-950/80 border border-slate-800 rounded-xl p-3 text-slate-200 text-xs font-mono focus:outline-none focus:border-indigo-500 placeholder:text-slate-700 leading-relaxed"
                  />
                  <div className="flex items-center gap-3 mt-3">
                    <button
                      ...
                      className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-slate-100 transition-all font-bold text-xs rounded-xl border border-indigo-500/40 shadow-md cursor-pointer"
                    >
                      Phân Tích Cấu Trúc
                    </button>
                    {parseResult && parseResult.draws.length > 0 && (
                      <button
                        type="button"
                        onClick={handleBulkImportSubmit}
                        className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-slate-100 transition-all font-bold text-xs rounded-xl border border-emerald-500/40 shadow-md flex items-center gap-1 cursor-pointer animate-pulse"
                      >
                        <Plus className="w-3.5 h-3.5" /> Xác Nhận Nhập ({parseResult.draws.length} kỳ)
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div className="lg:col-span-5 flex flex-col justify-between">
                  {parseResult ? (
                    <div className="bg-slate-950/60 rounded-xl p-4 border border-slate-800/80 h-full flex flex-col justify-between max-h-[220px] overflow-y-auto">
                      <div>
                        <h4 className="text-xs font-black text-slate-300 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                          <FileText className="w-3.5 h-3.5 text-indigo-400" /> Báo Cáo Phân Tích
                        </h4>
                        <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                          <div className="bg-slate-900/60 p-2 rounded-lg border border-slate-800/40">
                            <span className="text-[10px] text-slate-500 block">Dòng thô:</span>
                            <span className="font-bold font-mono text-slate-200">{parseResult.stats.totalLines}</span>
                          </div>
                          <div className="bg-slate-900/60 p-2 rounded-lg border border-slate-800/40">
                            <span className="text-[10px] text-slate-500 block">Trích xuất:</span>
                            <span className="font-bold font-mono text-emerald-400">{parseResult.draws.length}</span>
                          </div>
                        </div>

                        {parseResult.draws.length > 0 && (
                          <div className="space-y-1 mb-3">
                            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Xem trước 3 dòng đầu:</span>
                            {parseResult.draws.slice(0, 3).map((d: any, i: number) => (
                              <div key={i} className="text-[10px] font-mono bg-slate-900/40 p-1 rounded px-2 text-slate-400 flex justify-between">
                                <span>Kỳ #{d.id}</span>
                                <span className="text-indigo-400 font-bold">{d.numbers.join("-")}</span>
                              </div>
                            ))}
                          </div>
                        )}

                        {parseResult.stats.failedLines.length > 0 && (
                          <div className="mt-2">
                            <span className="text-[10px] text-rose-400 font-bold uppercase tracking-wider block flex items-center gap-1">
                              <AlertTriangle className="w-3 h-3 text-rose-400" /> Có {parseResult.stats.failedLines.length} lỗi phân tích:
                            </span>
                            <div className="max-h-[60px] overflow-y-auto text-[9px] font-mono text-slate-500 space-y-1 mt-1 bg-rose-950/10 p-1.5 rounded border border-rose-950/20">
                              {parseResult.stats.failedLines.map((f: any, i: number) => (
                                <div key={i} className="truncate">
                                  Dòng {f.lineNumber}: &quot;{f.line}&quot; &rarr; {f.error}
                                </div>
                              ))}
                            </div>
                          </div>
                    ...
                    <div className="bg-slate-950/30 rounded-xl border border-dashed border-slate-800 p-6 text-center flex flex-col items-center justify-center h-full text-slate-600">
                      <FileText className="w-8 h-8 text-slate-800 mb-2" />
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Báo cáo trống</p>
                      <p className="text-[10px] text-slate-600 mt-1 max-w-[200px]">Hãy dán văn bản thô và nhấn Phân Tích Cấu Trúc để bắt đầu.</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          {manualData.length > 0 && (
            <div className="mt-4 pt-4 border-t border-slate-800/60 flex justify-between items-center text-[11px] text-slate-500 font-medium">
              <span>
                Có {manualData.length} kỳ quay tự nhập trong cơ sở dữ liệu cục
                bộ.
              </span>
        ...
        <div className="block md:hidden divide-y divide-slate-800/45">
          {currentTableData.length > 0 ? (
            currentTableData.map((draw) => {
              const sum = getDrawSum(draw);
              const type = getSumType(sum);

              return (
                <div
                  key={draw.id}
                  className="p-4 flex flex-col gap-3 hover:bg-slate-800/10 transition-colors"
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="font-extrabold text-slate-200 font-mono text-base">
                        #{draw.id}
                      </span>
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`inline-block px-2.5 py-1 rounded font-extrabold text-[10px] uppercase border shadow-sm ${
                          type === "TAI"
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-6">
                        <div className="font-extrabold text-slate-200 font-mono text-sm">
                          #{draw.id}
                        </div>
                      ...
                      <td className="py-3 px-6 text-center">
                        {draw.isManual ? (
                          <span className="inline-block px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-400 border border-emerald-500/20 text-[9px] uppercase font-bold">
                            Nhập tay
                          </span>
                        ) : (
                          <span className="inline-block px-2 py-0.5 rounded bg-slate-800 text-slate-500 border border-slate-700/50 text-[9px] uppercase font-bold">
                            Hệ thống API
                          </span>
                        )}
                      </td>
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-t border-slate-800 bg-slate-950/20">
            <span className="text-[11px] text-slate-500 font-semibold">
              Đang xem Trang {currentPage} / {totalPages} (Tìm thấy{" "}
