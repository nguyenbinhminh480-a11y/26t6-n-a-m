...
export function validatePredictionOutput(payload: any): boolean {
  ...
  if (
    typeof payload.predictedSum !== "number" ||
    payload.predictedSum < 3 ||
    payload.predictedSum > 18
  )
    return false;
...
export abstract class BasePredictiveAgent {
   ...
   */
  public abstract generatePrediction(
    history: Draw[],
  ):
  ...
  public async execute(history: Draw[]): Promise<PredictionOutput> {
    ...
    try {
      if (!this.isHealthy) {
        throw new Error(
          `Tác nhân ${this.meta.name} đang trong trạng thái lỗi.`,
        );
      }
      ...
      const output: PredictionOutput = {
        predictedType:
          result.predictedSum >= 12
            ? "TAI"
            : result.predictedSum >= 10
              ? "HOA"
              : "XIU",
      ...
      this.lastExecutionTimeMs = Number(
  ...
  public getHealthReport(): HealthReport {
    ...
    const latencyAvg =
      this.latencyHistory.length > 0
        ? totalLatency / this.latencyHistory.length
        : 0;
  ...
  public static runAudit(): {
    ...
    agents.forEach((agent) => {
      ...
      if (report.status === "FAILED") {
        ...
        actionsTaken.push(
          "🚨 Sửa lỗi tự động: Kích hoạt khôi phục cứng tác nhân bị hỏng.",
        );
      } else if (report.status === "DEGRADED") {
        if (report.latencyAvgMs > 100) {
          actionsTaken.push(
            "⚠️ Cảnh báo: Phát hiện độ trễ tính toán vượt ngưỡng tối ưu. Bắt đầu thu hẹp kích thước lag.",
          );
        }
      }
