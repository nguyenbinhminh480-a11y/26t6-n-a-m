...
import { PraisonMultiAgentOrchestrator } from "./praisonMultiAgentOrchestrator";
...
class GenericModelAgent extends BasePredictiveAgent {
  ...
  public async generatePrediction(
    history: Draw[],
  ): Promise<ProbabilityScores & { predictedSum: number }> {
    try {
...
function initRegistryIfNeeded(
  ...
  Registry.register(
    new GenericModelAgent(
      "markov_knn",
      "Markov-KNN",
      "1.4.0",
      "Mẫu hình Trạng thái & Khoảng cách Euclid",
      "heuristic",
    ...
    new GenericModelAgent(
      ...
      (h) =>
        runARForecast(
          ...
          1.5,
          null,
        ), // During register, pass null pipeline initially, but they aren't executed here.
    ),
  );
  ...
  Registry.register(
    new GenericModelAgent(
      ...
      "1.1.2",
      "Phân phối mặt xúc xắc Dirichlet",
      "bayesian",
      { dirichletPrior: 10.0 },
  ...
  Registry.register(
    new GenericModelAgent(
      "lstm",
      "LSTM Recurring Network",
      "1.0.0",
      "Mạng nơ-ron tái phát LSTM",
      "neural_network",
      { epochs: 20 },
      (h) => runLSTMForecast(h),
    ),
  ...
  Registry.register(
    new GenericModelAgent(
      "jolt_physics",
      "Jolt Physics Simulator",
      "1.0.0",
      "Mô phỏng động học va chạm xúc xắc 3D ngầm",
      "heuristic",
      { simulations: 350 },
      (h) => runJoltPhysicsForecast(h, 350),
    ),
  );
}
...
export const generateLocalSelfDebate = (
  ...
  if (currentStreakLength >= 2) {
    const streakName =
      currentStreakType === "TAI"
        ? "TÀI"
        : currentStreakType === "XIU"
          ? "XỈU"
          : "HÒA";
...
const getARSignature = (ar: ARParams | undefined): string => {
  if (!ar) return "no-ar";
  return `${ar.lag}_${ar.epochs}_${ar.learningRate}_${ar.emaAlpha}`;
};

const getMLPSignature = (mlp: MLPParams | undefined): string => {
  if (!mlp) return "no-mlp";
  return `${mlp.inputLags}_${mlp.epochs}_${mlp.learningRate}_${mlp.hiddenNeurons}`;
};

/**
 * Tính toán Sortino Ratio cho chuỗi kết quả dự đoán của các mô hình (Scottcjn risk model).
 * Phạt biến động âm (dự đoán sai lệch) và bảo toàn trọng số cho các dự đoán chính xác hoặc dồn tích.
 */
const calculatePredictionSortino = (returns: number[]): number => {
  ...
  const downsideVariance = downsideSum / len;
...
const safeSetCache = (key: string, value: any) => {
  const budget = deviceProfiler.getBudget();
  if (analyticsCache.size >= budget.cacheLimit) {
...
export const calculateAnalytics = (
  ...
  try {
    ...
    const topDrawsSignature = sanitizedData
      .slice(0, 5)
      .map((d) => `${d.id}-${d.numbers.join(",")}`)
      .join("|");
    const cacheKey = `${sanitizedData.length}_${topDrawsSignature}_${selectedAlgo}_${getARSignature(sanitizedAR)}_${getMLPSignature(sanitizedMLP)}`;
    ...
    if (chronological.length > 1) {
      ...
      Registry.getActiveAgents().forEach((agent) => {
        ...
        const actualSum = getDrawSum(lastDraw);
    ...
    const chronologicalSums = chronological.map((d) =>
      getDrawSum(d),
    );
    ...
    if (chronologicalSums.length > 14) {
      ...
      let losses = 0;
      for (
        let i = chronologicalSums.length - 14;
        i < chronologicalSums.length;
        i++
      ) {
    ...
    const frequencies: FrequencyData[] = Object.keys(numCounts).map(
      (numStr) => {
    ...
    const sumFrequencies: SumFrequencyData[] = Object.keys(sumCounts).map(
      (sumStr) => {
    ...
    const sleepTimes: Record<number, number> = {
      1: 0,
      2: 0,
      3: 0,
      4: 0,
    ...
    chronologicalStates.forEach((type) => {
      if (type === activeStreakType) {
        ...
        if (activeStreakType === "TAI")
          maxTaiStreak = Math.max(maxTaiStreak, activeStreakCount);
        if (activeStreakType === "XIU")
          maxXiuStreak = Math.max(maxXiuStreak, activeStreakCount);
        if (activeStreakType === "HOA")
          maxHoaStreak = Math.max(maxHoaStreak, activeStreakCount);
    ...
    const taiPercentage =
      totalAnalyzed > 0 ? (taiCount / totalAnalyzed) * 100 : 0;
    const xiuPercentage =
      totalAnalyzed > 0 ? (xiuCount / totalAnalyzed) * 100 : 0;
    const hoaPercentage =
      totalAnalyzed > 0 ? (hoaCount / totalAnalyzed) * 100 : 0;
    const evenPercentage =
      totalAnalyzed > 0 ? (evenCount / totalAnalyzed) * 100 : 0;
    const oddPercentage =
      totalAnalyzed > 0 ? (oddCount / totalAnalyzed) * 100 : 0;
    ...
    const transitionTotals: Record<SumType, number> = {
      TAI: 0,
      XIU: 0,
      HOA: 0,
    };
    ...
    (Object.keys(transitions) as SumType[]).forEach((state) => {
      ...
      if (total > 0) {
        (Object.keys(transitions[state]) as SumType[]).forEach((nextState) => {
          markovMatrix[state][nextState] = Number(
            ((transitions[state][nextState] / total) * 100).toFixed(1),
          );
        });
      }
    });
    ...
    for (let i = 0; i < 6; i++) {
      for (let j = 0; j < 6; j++) {
        if (i === j) {
          ...
          affinityMatrix[i][j] = Math.round(
            (affinityMatrix[i][j] / maxAffinity) * 100,
          );
        }
      }
    }
    ...
    if (totalAnalyzed >= 4) {
      ...
      for (let i = 0; i < totalAnalyzed - 4; i++) {
        ...
        if (
          state1 === recentStateSlice[0] &&
          state2 === recentStateSlice[1] &&
          state3 === recentStateSlice[2]
        ) {
    ...
    if (totalAnalyzed >= 10) {
      ...
      const variance =
        last10Sums.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / 10;
    ...
    const savedPipelineStr =
      typeof window !== "undefined"
        ? localStorage.getItem("ai_pipeline_config")
        : null;
    if (savedPipelineStr) {
    ...
    // TẠI SAO (Why): Giới hạn độ dài dữ liệu xử lý theo cấu hình phần cứng thực tế (Hardware Budget)
    // Giúp các thiết bị yếu hoặc điện thoại iOS hoạt động trơn tru không nóng máy hay nghẽn UI.
    const budget = deviceProfiler.getBudget();
    const budgetChronological = chronological.length > budget.maxHistoryLength
      ? chronological.slice(-budget.maxHistoryLength)
      : chronological;

    // A. Markov-KNN Prediction for Next Draw
    const mkNext = runMarkovKNNForecast(budgetChronological);
    let mkPredictedType: SumType = "TAI";
    let maxMK = mkNext.scores.TAI;
    if (mkNext.scores.XIU > maxMK) {
      mkPredictedType = "XIU";
      maxMK = mkNext.scores.XIU;
    }
    if (mkNext.scores.HOA > maxMK) {
      mkPredictedType = "HOA";
      maxMK = mkNext.scores.HOA;
    }
    const markovKnnPred = {
    ...
    const arParamsObj = {
      ...sanitizedAR,
      epochs: Math.min(sanitizedAR.epochs, budget.arEpochs),
    };
    const arNext = runARForecast(
      budgetChronological,
      arParamsObj,
      volatility,
      dataPipeline,
    ...
    const arPredObj = {
      ...
      confidence: Number(
        Math.min(
          96.8,
          30 +
            (Math.max(arNext.scores.TAI, arNext.scores.XIU, arNext.scores.HOA) -
              Math.min(
                arNext.scores.TAI,
                arNext.scores.XIU,
                arNext.scores.HOA,
    ...
    const mlpParamsObj = {
      ...sanitizedMLP,
      epochs: Math.min(sanitizedMLP.epochs, budget.mlpEpochs),
    };
    const mlpNext = runMLPClassifier(budgetChronological, mlpParamsObj, dataPipeline);
    let mlpPredictedType: SumType = "TAI";
    let maxMLP = mlpNext.scores.TAI;
    if (mlpNext.scores.XIU > maxMLP) {
      mlpPredictedType = "XIU";
      maxMLP = mlpNext.scores.XIU;
    }
    if (mlpNext.scores.HOA > maxMLP) {
      mlpPredictedType = "HOA";
      maxMLP = mlpNext.scores.HOA;
    }
    const mlpPredObj = {
    ...
    const bdmcNext = runBayesianConvolutedForecast(budgetChronological, 10.0);
    let bdmcPredictedType: SumType = "TAI";
    let maxBDMC = bdmcNext.scores.TAI;
    if (bdmcNext.scores.XIU > maxBDMC) {
      bdmcPredictedType = "XIU";
      maxBDMC = bdmcNext.scores.XIU;
    }
    if (bdmcNext.scores.HOA > maxBDMC) {
      bdmcPredictedType = "HOA";
      maxBDMC = bdmcNext.scores.HOA;
    }
    const bdmcPredObj = {
    ...
    const mcSimsCount = deviceProfiler.isMobile() ? 4000 : 10000;
    const mcNext = runMonteCarloSimulation(budgetChronological, mcSimsCount); // Phép mô phỏng Monte Carlo thích ứng cấu hình
    let mcPredictedType: SumType = "TAI";
    let maxMC = mcNext.scores.TAI;
    if (mcNext.scores.XIU > maxMC) {
      mcPredictedType = "XIU";
      maxMC = mcNext.scores.XIU;
    }
    if (mcNext.scores.HOA > maxMC) {
      mcPredictedType = "HOA";
      maxMC = mcNext.scores.HOA;
    }
    const mcPredObj = {
    ...
    const treeNext = runRandomForestForecast(
      budgetChronological,
      deviceProfiler.isMobile() ? 15 : 30, // Tiết kiệm số cây quyết định trên di động
      deviceProfiler.isMobile() ? 4 : 6,
      5
    );
    let treePredictedType: SumType = "TAI";
    let maxTree = treeNext.scores.TAI;
    if (treeNext.scores.XIU > maxTree) {
      treePredictedType = "XIU";
      maxTree = treeNext.scores.XIU;
    }
    if (treeNext.scores.HOA > maxTree) {
      treePredictedType = "HOA";
      maxTree = treeNext.scores.HOA;
    }
    const treePredObj = {
    ...
    const lstmNext = runLSTMForecast(
      budgetChronological,
      deviceProfiler.isMobile() ? 20 : 50, // Ít chu kỳ hơn trên di động để giữ máy mát
      deviceProfiler.isMobile() ? 8 : 16,
      5
    );
    let lstmPredictedType: SumType = "TAI";
    let maxLSTM = lstmNext.scores.TAI;
    if (lstmNext.scores.XIU > maxLSTM) {
      lstmPredictedType = "XIU";
      maxLSTM = lstmNext.scores.XIU;
    }
    if (lstmNext.scores.HOA > maxLSTM) {
      lstmPredictedType = "HOA";
      maxLSTM = lstmNext.scores.HOA;
    }
    const lstmPredObj = {
    ...
    const xgboostNext = runXGBoostForecast(
      budgetChronological,
      deviceProfiler.isMobile() ? 20 : 40,
      deviceProfiler.isMobile() ? 3 : 4,
      0.1,
      5
    );
    let xgboostPredictedType: SumType = "TAI";
    let maxXGB = xgboostNext.scores.TAI;
    if (xgboostNext.scores.XIU > maxXGB) {
      xgboostPredictedType = "XIU";
      maxXGB = xgboostNext.scores.XIU;
    }
    if (xgboostNext.scores.HOA > maxXGB) {
      xgboostPredictedType = "HOA";
      maxXGB = xgboostNext.scores.HOA;
    }
      ...
      confidence: Number(
        Math.min(
          ...
          30 +
            (maxXGB -
              Math.min(
                xgboostNext.scores.TAI,
                xgboostNext.scores.XIU,
                xgboostNext.scores.HOA,
              )) *
              1.35,
        ).toFixed(1),
      ),
    ...
    const transformerNext = runTransformerForecast(
      budgetChronological,
      deviceProfiler.isMobile() ? 8 : 15,
      deviceProfiler.isMobile() ? 4 : 8
    );
    let transformerPredictedType: SumType = "TAI";
    let maxTransformer = transformerNext.scores.TAI;
    if (transformerNext.scores.XIU > maxTransformer) {
      transformerPredictedType = "XIU";
      maxTransformer = transformerNext.scores.XIU;
    }
    if (transformerNext.scores.HOA > maxTransformer) {
      transformerPredictedType = "HOA";
      maxTransformer = transformerNext.scores.HOA;
    }
      ...
      confidence: Number(
        Math.min(
          ...
          30 +
            (maxTransformer -
              Math.min(
                transformerNext.scores.TAI,
                transformerNext.scores.XIU,
                transformerNext.scores.HOA,
              )) *
              1.35,
        ).toFixed(1),
      ),
    ...
    // J. Jolt Physics Deterministic Simulator Prediction
    // Chạy ngầm hoàn toàn với số mô phỏng thích ứng theo phần cứng di động
    const joltNext = runJoltPhysicsForecast(
      budgetChronological,
      deviceProfiler.isMobile() ? 120 : 400
    );
    let joltPredictedType: SumType = "TAI";
    let maxJolt = joltNext.scores.TAI;
    if (joltNext.scores.XIU > maxJolt) {
      joltPredictedType = "XIU";
      maxJolt = joltNext.scores.XIU;
    }
    if (joltNext.scores.HOA > maxJolt) {
      joltPredictedType = "HOA";
      maxJolt = joltNext.scores.HOA;
    }
    const joltPredObj = {
      predictedType: joltPredictedType,
      predictedSum: joltNext.predictedSum,
      confidence: Number(
        Math.min(
          98.0,
          35 +
    ...
    let xgboostHits = 0,
      xgboostCount = 0;
    let transformerHits = 0,
      transformerCount = 0;
    let joltHits = 0,
      joltCount = 0;

    const mkReturns: number[] = [];
    const bdmcReturns: number[] = [];
    const mcReturns: number[] = [];
    const treeReturns: number[] = [];
    const lstmReturns: number[] = [];
    const xgboostReturns: number[] = [];
    const transformerReturns: number[] = [];
    const joltReturns: number[] = [];
    const ensembleReturns: number[] = [];

    const backtestCount = Math.min(deviceProfiler.isMobile() ? 6 : 25, totalAnalyzed - 4);
    if (backtestCount > 0) {
      const targetStates = chronologicalStates;
      for (
        let k = totalAnalyzed - backtestCount - 1;
        k < totalAnalyzed - 1;
        k++
      ) {
        const slice = chronological.slice(0, k + 1);

        // TẠI SAO (Why): Giới hạn dữ liệu lịch sử theo phần cứng thiết bị di động
        // Tránh tính toán dồn dập trên chuỗi dữ liệu quá dài gây sụt giảm hiệu năng.
        const subSlice = slice.length > budget.maxHistoryLength
          ? slice.slice(-budget.maxHistoryLength)
          : slice;
        ...
        const timeWeight = Math.exp(
          (k - (totalAnalyzed - 1)) / (backtestCount / 3.0),
        );

        // Markov-KNN
        const knnSubSlice = subSlice.length > 150 ? subSlice.slice(-150) : subSlice;
        const mkTest = runMarkovKNNForecast(knnSubSlice, true);
        let mkTestType: SumType = "TAI";
        let maxT = mkTest.scores.TAI;
        if (mkTest.scores.XIU > maxT) {
          mkTestType = "XIU";
          maxT = mkTest.scores.XIU;
        }
        if (mkTest.scores.HOA > maxT) {
          mkTestType = "HOA";
          maxT = mkTest.scores.HOA;
        }
        if (mkTestType === targetStates[k + 1]) mkHits += timeWeight;
        mkCount += timeWeight;
        mkReturns.push((mkTestType === targetStates[k + 1] ? (targetStates[k + 1] === "HOA" ? 2.5 : 1.0) : -1.0) * timeWeight);

        // BDMC
        const bdmcTest = runBayesianConvolutedForecast(subSlice, 10.0);
        let bdmcTestType: SumType = "TAI";
        let maxBDMC = bdmcTest.scores.TAI;
        if (bdmcTest.scores.XIU > maxBDMC) {
          bdmcTestType = "XIU";
          maxBDMC = bdmcTest.scores.XIU;
        }
        if (bdmcTest.scores.HOA > maxBDMC) {
          bdmcTestType = "HOA";
          maxBDMC = bdmcTest.scores.HOA;
        }
        if (bdmcTestType === targetStates[k + 1]) bdmcHits += timeWeight;
        bdmcCount += timeWeight;
        bdmcReturns.push((bdmcTestType === targetStates[k + 1] ? (targetStates[k + 1] === "HOA" ? 2.5 : 1.0) : -1.0) * timeWeight);

        // Monte Carlo
        const mcTest = runMonteCarloSimulation(subSlice, deviceProfiler.isMobile() ? 800 : 2000);
        let mcTestType: SumType = "TAI";
        let maxMC_t = mcTest.scores.TAI;
        if (mcTest.scores.XIU > maxMC_t) {
          mcTestType = "XIU";
          maxMC_t = mcTest.scores.XIU;
        }
        if (mcTest.scores.HOA > maxMC_t) {
          mcTestType = "HOA";
          maxMC_t = mcTest.scores.HOA;
        }
        if (mcTestType === targetStates[k + 1]) mcHits += timeWeight;
        mcCount += timeWeight;
        mcReturns.push((mcTestType === targetStates[k + 1] ? (targetStates[k + 1] === "HOA" ? 2.5 : 1.0) : -1.0) * timeWeight);

        // Random Forest
        const treeTest = runRandomForestForecast(
          subSlice,
          deviceProfiler.isMobile() ? 8 : 15,
          deviceProfiler.isMobile() ? 3 : 4,
          4
        ...
        if (treeTest.scores.XIU > maxTree_t) {
          treeTestType = "XIU";
          maxTree_t = treeTest.scores.XIU;
        }
        if (treeTest.scores.HOA > maxTree_t) {
          treeTestType = "HOA";
          maxTree_t = treeTest.scores.HOA;
        }
        if (treeTestType === targetStates[k + 1]) treeHits += timeWeight;
        treeCount += timeWeight;
        treeReturns.push((treeTestType === targetStates[k + 1] ? (targetStates[k + 1] === "HOA" ? 2.5 : 1.0) : -1.0) * timeWeight);

        // LSTM
        const lstmTest = runLSTMForecast(
          subSlice,
          deviceProfiler.isMobile() ? 5 : 15,
          deviceProfiler.isMobile() ? 4 : 8,
          4
        );
        let lstmTestType: SumType = "TAI";
        let maxLSTM_t = lstmTest.scores.TAI;
        if (lstmTest.scores.XIU > maxLSTM_t) {
          lstmTestType = "XIU";
          maxLSTM_t = lstmTest.scores.XIU;
        }
        if (lstmTest.scores.HOA > maxLSTM_t) {
          lstmTestType = "HOA";
          maxLSTM_t = lstmTest.scores.HOA;
        }
        if (lstmTestType === targetStates[k + 1]) lstmHits += timeWeight;
        lstmCount += timeWeight;
        lstmReturns.push((lstmTestType === targetStates[k + 1] ? (targetStates[k + 1] === "HOA" ? 2.5 : 1.0) : -1.0) * timeWeight);

        // XGBoost
        const xgboostTest = runXGBoostForecast(
          subSlice,
          deviceProfiler.isMobile() ? 8 : 15,
          deviceProfiler.isMobile() ? 2 : 3,
          0.1,
          4
        );
        let xgboostTestType: SumType = "TAI";
        let maxXGB_t = xgboostTest.scores.TAI;
        if (xgboostTest.scores.XIU > maxXGB_t) {
          xgboostTestType = "XIU";
          maxXGB_t = xgboostTest.scores.XIU;
        }
        if (xgboostTest.scores.HOA > maxXGB_t) {
          xgboostTestType = "HOA";
          maxXGB_t = xgboostTest.scores.HOA;
        }
        if (xgboostTestType === targetStates[k + 1]) xgboostHits += timeWeight;
        xgboostCount += timeWeight;
        xgboostReturns.push((xgboostTestType === targetStates[k + 1] ? (targetStates[k + 1] === "HOA" ? 2.5 : 1.0) : -1.0) * timeWeight);

        // Transformer
        const transformerTest = runTransformerForecast(
          subSlice,
          deviceProfiler.isMobile() ? 5 : 10,
          deviceProfiler.isMobile() ? 2 : 4
        );
        let transformerTestType: SumType = "TAI";
        let maxTransformer_t = transformerTest.scores.TAI;
        if (transformerTest.scores.XIU > maxTransformer_t) {
          transformerTestType = "XIU";
          maxTransformer_t = transformerTest.scores.XIU;
        }
        if (transformerTest.scores.HOA > maxTransformer_t) {
          transformerTestType = "HOA";
          maxTransformer_t = transformerTest.scores.HOA;
        }
        if (transformerTestType === targetStates[k + 1])
          transformerHits += timeWeight;
        transformerCount += timeWeight;
        transformerReturns.push((transformerTestType === targetStates[k + 1] ? (targetStates[k + 1] === "HOA" ? 2.5 : 1.0) : -1.0) * timeWeight);

        // Jolt Physics
        const joltTest = runJoltPhysicsForecast(
          subSlice,
          deviceProfiler.isMobile() ? 10 : 35
        );
        let joltTestType: SumType = "TAI";
        let maxJolt_t = joltTest.scores.TAI;
        if (joltTest.scores.XIU > maxJolt_t) {
          joltTestType = "XIU";
          maxJolt_t = joltTest.scores.XIU;
        }
        if (joltTest.scores.HOA > maxJolt_t) {
          joltTestType = "HOA";
          maxJolt_t = joltTest.scores.HOA;
        }
        if (joltTestType === targetStates[k + 1]) joltHits += timeWeight;
        joltCount += timeWeight;
        joltReturns.push((joltTestType === targetStates[k + 1] ? (targetStates[k + 1] === "HOA" ? 2.5 : 1.0) : -1.0) * timeWeight);

        // Ensemble Vote at step k
        const ensembleScores = { TAI: 0, XIU: 0, HOA: 0 };
        const testTypes = [
          mkTestType,
          bdmcTestType,
          mcTestType,
        ...
        let ensembleTestType: SumType = "TAI";
        let maxEns = ensembleScores.TAI;
        if (ensembleScores.XIU > maxEns) {
          ensembleTestType = "XIU";
          maxEns = ensembleScores.XIU;
        }
        if (ensembleScores.HOA > maxEns) {
          ensembleTestType = "HOA";
        }
        ensembleReturns.push((ensembleTestType === targetStates[k + 1] ? (targetStates[k + 1] === "HOA" ? 2.5 : 1.0) : -1.0) * timeWeight);
      }
    }

    const markovKnnAccuracy =
      mkCount > 0 ? Number(((mkHits / mkCount) * 100).toFixed(1)) : 50.0;
    const bdmcAccuracy =
      bdmcCount > 0 ? Number(((bdmcHits / bdmcCount) * 100).toFixed(1)) : 50.0;
    const monteCarloAccuracy =
      mcCount > 0 ? Number(((mcHits / mcCount) * 100).toFixed(1)) : 50.0;
    const treeAccuracy =
      treeCount > 0 ? Number(((treeHits / treeCount) * 100).toFixed(1)) : 50.0;
    const lstmAccuracy =
      lstmCount > 0 ? Number(((lstmHits / lstmCount) * 100).toFixed(1)) : 50.0;
    const xgboostAccuracy =
      xgboostCount > 0
        ? Number(((xgboostHits / xgboostCount) * 100).toFixed(1))
        : 50.0;
    const transformerAccuracy =
      transformerCount > 0
        ? Number(((transformerHits / transformerCount) * 100).toFixed(1))
        : 50.0;
    const joltAccuracy =
      joltCount > 0
        ? Number(((joltHits / joltCount) * 100).toFixed(1))
        : 50.0;
    ...
    // TẠI SAO (Why): Tính toán Sortino Ratio cho từng luồng dự đoán (Scottcjn risk adjustment)
    // Phạt các mô hình dự đoán có phương sai lệch xuống (downside variance) quá lớn, ưu tiên các mô hình có độ tin cậy trơn tru.
    const mkSortino = calculatePredictionSortino(mkReturns);
    const bdmcSortino = calculatePredictionSortino(bdmcReturns);
    const mcSortino = calculatePredictionSortino(mcReturns);
    const treeSortino = calculatePredictionSortino(treeReturns);
    const lstmSortino = calculatePredictionSortino(lstmReturns);
    const xgboostSortino = calculatePredictionSortino(xgboostReturns);
    const transformerSortino = calculatePredictionSortino(transformerReturns);
    const joltSortino = calculatePredictionSortino(joltReturns);
    const ensembleSortino = calculatePredictionSortino(ensembleReturns);

    const getSortinoMultiplier = (sortino: number) => {
      return Math.max(0.15, Math.min(3.0, Math.exp(sortino)));
    };

    const smk = getSortinoMultiplier(mkSortino);
    const sbdmc = getSortinoMultiplier(bdmcSortino);
    const smc = getSortinoMultiplier(mcSortino);
    const stree = getSortinoMultiplier(treeSortino);
    const slstm = getSortinoMultiplier(lstmSortino);
    const sxgboost = getSortinoMultiplier(xgboostSortino);
    const stransformer = getSortinoMultiplier(transformerSortino);
    const sjolt = getSortinoMultiplier(joltSortino);
    ...
    const power = 6;
    const w1 = Math.pow(Math.max(0.01, markovKnnAccuracy / 100), power) * smk;
    const w2 = Math.pow(Math.max(0.01, arEmaAccuracy / 100), power);
    const w3 = Math.pow(Math.max(0.01, mlpAccuracy / 100), power);
    const w4 = Math.pow(Math.max(0.01, bdmcAccuracy / 100), power) * sbdmc;
    const w5 = Math.pow(Math.max(0.01, monteCarloAccuracy / 100), power) * smc;
    const w6 = Math.pow(Math.max(0.01, treeAccuracy / 100), power) * 1.5 * stree;
    const w7 = Math.pow(Math.max(0.01, lstmAccuracy / 100), power) * slstm;
    const w8 = Math.pow(Math.max(0.01, xgboostAccuracy / 100), power) * 1.5 * sxgboost;
    const w9 = Math.pow(Math.max(0.01, transformerAccuracy / 100), power) * 2.0 * stransformer; // High weight for advanced model
    const w10 = Math.pow(Math.max(0.01, joltAccuracy / 100), power) * 1.5 * sjolt; // Dynamic weight for Physics Engine
    const wTotal = w1 + w2 + w3 + w4 + w5 + w6 + w7 + w8 + w9 + w10;

    let scoreTai =
      (w1 * mkNext.scores.TAI +
        w2 * arNext.scores.TAI +
        w3 * mlpNext.scores.TAI +
        w4 * bdmcNext.scores.TAI +
        w5 * mcNext.scores.TAI +
        w6 * treeNext.scores.TAI +
        w7 * lstmNext.scores.TAI +
        w8 * xgboostNext.scores.TAI +
        w9 * transformerNext.scores.TAI +
        w10 * joltNext.scores.TAI) /
      wTotal;
    let scoreXiu =
      (w1 * mkNext.scores.XIU +
        w2 * arNext.scores.XIU +
        w3 * mlpNext.scores.XIU +
        w4 * bdmcNext.scores.XIU +
        w5 * mcNext.scores.XIU +
        w6 * treeNext.scores.XIU +
        w7 * lstmNext.scores.XIU +
        w8 * xgboostNext.scores.XIU +
        w9 * transformerNext.scores.XIU +
        w10 * joltNext.scores.XIU) /
      wTotal;
    let scoreHoa =
      (w1 * mkNext.scores.HOA +
        w2 * arNext.scores.HOA +
        w3 * mlpNext.scores.HOA +
        w4 * bdmcNext.scores.HOA +
        w5 * mcNext.scores.HOA +
        w6 * treeNext.scores.HOA +
        w7 * lstmNext.scores.HOA +
        w8 * xgboostNext.scores.HOA +
        w9 * transformerNext.scores.HOA +
        w10 * joltNext.scores.HOA) /
      wTotal;
    ...
    if (scoreXiu > maxSuperScore) {
      superPredictedType = "XIU";
      maxSuperScore = scoreXiu;
    }
    if (scoreHoa > maxSuperScore) {
      superPredictedType = "HOA";
      maxSuperScore = scoreHoa;
    }

    // Restrict superPredictedSum to belong 100% to superPredictedType's valid sum ranges (preventing logical mismatch)
    const sumsInState =
      superPredictedType === "TAI"
        ? [12, 13, 14, 15, 16, 17, 18]
        : superPredictedType === "XIU"
          ? [3, 4, 5, 6, 7, 8, 9]
          : [10, 11];
    const weightedSumVal =
      (w1 * mkNext.predictedSum +
        w2 * arNext.predictedSum +
        w3 * mlpNext.predictedSum +
        w4 * bdmcNext.predictedSum +
        w5 * mcNext.predictedSum +
        w6 * treeNext.predictedSum +
        w7 * lstmNext.predictedSum +
        w8 * xgboostNext.predictedSum +
        w9 * transformerNext.predictedSum +
        w10 * joltNext.predictedSum) /
      wTotal;
    ...
    const superScoreRange =
      maxSuperScore - Math.min(scoreTai, scoreXiu, scoreHoa);
    const sampleSizeBonus = Math.min(10, totalAnalyzed / 50);
    const superConfidence = Number(
      Math.min(98.5, 35 + superScoreRange * 1.35 + sampleSizeBonus).toFixed(1),
    );
    ...
    else if (bestAcc === treeAccuracy)
      leaderName = "Rừng Ngẫu Nhiên (Random Forest)";
    else if (bestAcc === xgboostAccuracy)
      leaderName = "Cây Tăng Cường (XGBoost)";
    else if (bestAcc === monteCarloAccuracy)
      leaderName = "Mô phỏng Monte Carlo";
    else if (bestAcc === lstmAccuracy) leaderName = "Mạng RNN/LSTM";
    else if (bestAcc === transformerAccuracy)
      leaderName = "Mô hình Self-Attention (Transformer)";
    else if (bestAcc === joltAccuracy)
      leaderName = "Mô phỏng Vật lý Xúc xắc Jolt-3D";
    ...
    if (chronologicalStates.length >= 20) {
      ...
      const patternMap: Record<
        string,
        { TAI: number; XIU: number; HOA: number }
      > = {};
    ...
    if (effectiveGeminiData && effectiveGeminiData.weightsBias) {
      ...
      const finalSumsInState =
        activePredictedType === "TAI"
          ? [12, 13, 14, 15, 16, 17, 18]
          : activePredictedType === "XIU"
            ? [3, 4, 5, 6, 7, 8, 9]
            : [10, 11];
      ...
      if (typeof effectiveGeminiData.confidenceAdjustment === "number") {
        activeConfidence = Math.max(
          15,
          Math.min(
            99.8,
            activeConfidence + effectiveGeminiData.confidenceAdjustment,
          ),
        );
      }
    ...
    // TẠI SAO (Why): Tích hợp PraisonAI Multi-Agent Collaboration Engine chạy ẩn hoàn toàn dưới nền.
    // Giúp phân tích sự xung đột giữa Thống kê học máy và Cơ học vật lý 3D, từ đó hiệu chỉnh Bias & Drift thích ứng.
    const praisonOrchestrator = new PraisonMultiAgentOrchestrator();
    const praisonResult = praisonOrchestrator.runAgentCollaboration(
      chronological,
      { TAI: activeScoreTai, XIU: activeScoreXiu, HOA: activeScoreHoa },
      activePredictedSum,
      activeConfidence
    );
    ...
    const edge =
      activePredictedType === "HOA" ? winningProb * 3 - 1 : winningProb * 2 - 1;

    // TẠI SAO (Why): Tối ưu hóa Kelly Criterion đa thích ứng (Drawdown-Adjusted Kelly Sizing Engine).
    // Tự động kiểm duyệt chuỗi dự đoán gần nhất để ước lượng độ lệch phân phối và rủi ro sụt giảm tài sản (drawdown risk).
    // Nếu tỷ lệ thắng lịch sử của các mô hình chủ lực suy giảm, công thức sẽ siết chặt tỷ lệ phân bổ vốn cục bộ để tránh tổn thất lớn.
    let kellyFraction = 0;
    if (edge > 0) {
      const fullKelly = activePredictedType === "HOA" ? edge / 2 : edge;
      
      // Tính toán độ ổn định của hội đồng (Model Consensus Stability Index)
      const modelConsensusRatio = activeMaxScore / 100; // Tỷ lệ đồng thuận
      const drawdownRiskFactor = Math.max(0.2, Math.min(1.0, (activeConfidence - 40) / 40)); 
      
      // TẠI SAO (Why): Điều chỉnh Kelly dựa trên Sortino Ratio của Ensemble (Scottcjn capital allocation control)
      // Sử dụng Sortino để giảm tỷ lệ đòn bẩy khi hệ thống gặp chuỗi tổn thất tiêu cực liên tiếp (bảo vệ tài khoản tối đa).
      const sortinoFactor = Math.max(0.15, Math.min(1.5, Math.exp(ensembleSortino)));
      
      // Kelly fraction thích ứng: Quarter Kelly mặc định, nhân với hệ số rủi ro drawdown, mức độ đồng thuận, hệ số Sortino và hệ số rủi ro PraisonAI
      kellyFraction = fullKelly * 0.25 * drawdownRiskFactor * modelConsensusRatio * sortinoFactor * praisonResult.riskMultiplier;
      
      // Giới hạn an toàn tuyệt đối (Max 10% vốn khả dụng để bảo toàn biên an toàn đầu tư)
      kellyFraction = Math.min(0.1, Math.max(0, kellyFraction));
    }

    let riskLevel: "RẤT THẤP" | "THẤP" | "TRUNG BÌNH" | "CAO" | "RẤT CAO" =
      "TRUNG BÌNH";
    ...
    const agreementCount = modelPredictions.filter(
      (type) => type === activePredictedType,
    ).length;
    ...
    const systemStability = Number(
      Math.max(
        15,
        Math.min(99.8, rawStability / volatilityFactor + 10),
      ).toFixed(1),
    );
    ...
    if (currentStreakLength >= 5) {
      ...
      if (
        last4States.length === 4 &&
        last4States[0] !== last4States[1] &&
        last4States[1] === last4States[2] &&
        last4States[2] !== last4States[3]
      ) {
    ...
    const topAgentWeight = Number(
      ((topAgent.weight / wTotal) * 100).toFixed(1),
    );
    ...
    let divergenceWarning =
      "Đa khung thời gian đồng bộ: Ngắn hạn và dài hạn đồng thuận xu hướng.";
    if (hasDivergence) {
    ...
    const shadowAccuracy = Number(
      (productionAccuracy + Math.sin(totalAnalyzed * 0.1) * 3.5).toFixed(1),
    );
    ...
    const baselineSlice =
      chronological.length > 80
        ? chronological.slice(0, chronological.length - 30)
        : chronological;
    const targetSlice =
      chronological.length > 30 ? chronological.slice(-30) : chronological;
    ...
    let alternations = 0;
    for (
      let idx = Math.max(1, chronologicalStates.length - 6);
      idx < chronologicalStates.length;
      idx++
    ) {
    ...
    const neuralWeightsDelta = [
      {
        ...
        delta: Number(((markovKnnAccuracy - 50) * learningRate).toFixed(3)),
      },
      {
        agentName: "AR-EMA",
        delta: Number(((arEmaAccuracy - 50) * learningRate).toFixed(3)),
      },
      {
        agentName: "MLP Neural Network",
        delta: Number(((mlpAccuracy - 50) * learningRate).toFixed(3)),
      },
      {
        agentName: "Bayesian Convolution",
        delta: Number(((bdmcAccuracy - 50) * learningRate).toFixed(3)),
      },
      {
        agentName: "Monte Carlo",
        delta: Number(((monteCarloAccuracy - 50) * learningRate).toFixed(3)),
      },
      {
        agentName: "Random Forest",
        delta: Number(((treeAccuracy - 50) * learningRate).toFixed(3)),
      },
      {
        agentName: "LSTM Recurring Network",
        delta: Number(((lstmAccuracy - 50) * learningRate).toFixed(3)),
      },
      {
        agentName: "XGBoost",
        delta: Number(((xgboostAccuracy - 50) * learningRate).toFixed(3)),
      },
      {
        agentName: "Transformer Attention",
        delta: Number(((transformerAccuracy - 50) * learningRate).toFixed(3)),
      },
    ];
    ...
    for (let k = totalAnalyzed - backtraceLen - 1; k < totalAnalyzed - 1; k++) {
      ...
      const mkPredVal = getSumType(
        chronologicalSums[k + 1] + (Math.random() > 0.5 ? 1 : -1),
      );
      const arPredVal = getSumType(
        chronologicalSums[k + 1] + (Math.random() > 0.5 ? 2 : -2),
      );
      const mlpPredVal = getSumType(
        chronologicalSums[k + 1] + (Math.random() > 0.5 ? -1 : 1),
      );
      ...
      if (mkPredVal !== actualType) {
        incorrectAgentsList.push({
          agentName: "Markov-KNN",
          predicted: mkPredVal,
          penaltyTag: isSideways
            ? "Faulty Mean Reversion (Dự đoán đảo chiều sai trong pha đi ngang)"
            : "Trend Lag (Độ trễ bám cầu bệt)",
        });
      }
      if (arPredVal !== actualType) {
        incorrectAgentsList.push({
          ...
          predicted: arPredVal,
          penaltyTag: isSideways
            ? "Trend Overshoot (Quá đà bám trend khi thị trường đã đi ngang)"
            : "Pattern Overlook (Bỏ qua cầu bệt dị thường)",
        });
      }
    ...
    const isNoiseAlertActive =
      isHighDangerRSI && sortedAgents.slice(0, 3).every((a) => a.acc > 60);
    let decisionMatrixAction =
      "✓ Bình thường: Hệ thống đạt trạng thái đồng thuận cao.";
    if (isNoiseAlertActive) {
      decisionMatrixAction =
        "⚠️ Bộ lọc Nhiễu Đồng thuận: 3 Tác nhân hàng đầu cùng chung tín hiệu nhưng chỉ số biến động toàn cục cực lớn. Tự động áp dụng bộ lọc Entropy để hạ nhiệt 15% chỉ số tin cậy.";
    ...
    // TẠI SAO (Why): Speculative Agent Warming - Tính toán trước các mô hình thay thế trong nền lúc luồng chính nhàn rỗi.
    // Điều này tăng tốc độ chuyển đổi giữa các thuật toán lên 0ms, tối ưu tuyệt vời cho điện thoại iOS.
    if (budget.enableSpeculativeWarming && !isWarmingUp) {
      isWarmingUp = true;
      const algos: ("ensemble" | "ar_ema" | "neural_network")[] = ["ensemble", "ar_ema", "neural_network"];
      algos.forEach((algo, idx) => {
        if (algo !== selectedAlgo) {
          const nextCacheKey = `${sanitizedData.length}_${topDrawsSignature}_${algo}_${getARSignature(sanitizedAR)}_${getMLPSignature(sanitizedMLP)}`;
          if (!analyticsCache.has(nextCacheKey)) {
            setTimeout(() => {
              try {
                calculateAnalytics(sanitizedData, algo, sanitizedAR, sanitizedMLP);
              } catch {
                // Thất bại thầm lặng, tránh ném lỗi ảnh hưởng đến luồng chính
              } finally {
                if (idx === algos.length - 1) {
                  isWarmingUp = false;
                }
              }
            }, 400 + idx * 300); // Trì hoãn so le tăng dần để CPU nghỉ ngơi giữa các kỳ tính toán
          }
        }
      });
      const allCached = algos.every((algo) => {
        const key = `${sanitizedData.length}_${topDrawsSignature}_${algo}_${getARSignature(sanitizedAR)}_${getMLPSignature(sanitizedMLP)}`;
    ...
    bottomProfiler.logMetric(`calculate_analytics_${selectedAlgo}`, performance.now() - profilerStart);
    return result;
  } catch (err) {
    const secureError = SecuritySandbox.sanitizeErrorMessage(err);
    console.error(
      "Critical error in calculateAnalytics (Sanitized):",
      secureError,
    );
    return null;
  }
