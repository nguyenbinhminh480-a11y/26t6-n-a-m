...
export interface FinancialReportOutput {
  ...
  profitFactor: number;
  sharpeRatio: number;
  sortinoRatio: number; // TẠI SAO (Why): Tối ưu hóa chỉ số rủi ro trượt Sortino để kiểm soát sụt giảm tài sản (drawdown)
  ...
  try {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (
        ...
        typeof item.payoutAmount !== "number"
      ) {
    ...
    let sumSquaredDownside = 0; // Phục vụ tính toán độ lệch chuẩn giảm (Downside Deviation)
    for (let j = 0; j < totalTrades; j++) {
      const diff = returns[j] - meanReturn;
      sumSquaredDiffs += diff * diff;
      
      // Chỉ tính các biến động tiêu cực (dưới mức 0 hoặc phi rủi ro) cho Sortino Ratio
      if (returns[j] < 0) {
        sumSquaredDownside += returns[j] * returns[j];
      }
    }
    ...
    const sharpeRatio =
      stdDev === 0 ? 0 : Number((meanReturn / stdDev).toFixed(3));

    // Tính toán Downside Deviation và Sortino Ratio theo lý thuyết Scottcjn
    const downsideVariance = sumSquaredDownside / totalTrades;
    ...
    const sortinoRatio =
      downsideStdDev === 0
        ? meanReturn > 0
          ? 99.99
          : 0
        : Number((meanReturn / downsideStdDev).toFixed(3));
